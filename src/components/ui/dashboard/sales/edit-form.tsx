...
import { updateSale, autocompleteProducts } from './actions';
...
import { Product, PSale, PSaleProduct, SaleProduct } from '@/types/db';
import {
  Table,
  TableBody,
  TableCell,
...
export function EditSaleForm({ sale }: { sale: PSale }) {
  ...
  return (
    <Form {...form}>
      <form action={formAction} className="space-y-8">
        {isPending && <ScreenSpinner label="Updating sale" />}
...
function ProductsInput({
  ...
  >(() => {
    ...
    return initialProducts.map<SaleProductTableItem>(
      ({ quantity, ...sale }) => ({
        quantityString: quantity.toFixed(0),
        ...sale,
      })
    );
  });
